...
class TicketQRReader extends React.Component<Props, {readResultList: string[], scannerControls: IScannerControls | null}, {cameraRef: HTMLVideoElement}> {
  ...
  private numberOfReadCode = 2 // 読み取るQRコード数
    ...
    codeReader.decodeFromVideoDevice(
      ...
      (result, exception, controls) => {
        ...
        if (result) {
          ...
          if (resultArray.length === this.numberOfReadCode - 1) {
            // 読み取り数に達したら
  ...
  render() {
    ...
    // 読み取り状況の表示
    const readQRStatuses = Array(this.numberOfReadCode).fill(null).map((_, index) => {
      ...
      const deleteIconButton = result ?
        <Tooltip title="取り消し">
          <IconButton color="warning" size="small" sx={{p: 0}} onClick={() => this.clearReadResult()}>
